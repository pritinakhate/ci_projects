...
class Guests extends CI_Controller {
	...
	public function index($userid='')
	{
		...
		if($userid != '')
		{
			$condition = "guests.user_id='".$userid."'";
		}
		else{
			$condition = "guests.user_id='".$this->session->userdata('id')."'";
		}

		// Manage/List page from guests folder
		$allguests = $this->Guests_model->GetGuestsData("guests","guests.token,guests.id,guests.name,guests.email_address,guests.address,guests.details_about_guest,guests.dob,guests.gender,guests.photo,guests.status,users.name as username,guests.created,countries.country_name,states.state_name,cities.city_name,hobbies.hobby_title",$condition,"guests.created DESC");
		...
		if(isset($_POST) && !empty($_POST))
		{
			...
			}else
			{
				...
				else
				{
					...
					// start multiple file upload code
	...
	public function update($token)
	{
		...
		$getsingleguests = $this->Guests_model->GetData("guests","","token='".base64_decode($token)."'","","","","single");
		if(!empty($getsingleguests))
		{
			...
			$data = array(
				...
				'heading' => 'Guestbook',
				'screen' => 'Update Guest',
				'action' => site_url('Guests/update_action/'.$token),
				'button' => 'Update',
				'cancel' => 'Cancel',
				'cancel_action' => site_url('Guests/index'),
