...
class Home extends CI_Controller {
	...
	public function login_action()
	{
		...
		else
		{
			...
			if(empty($checkemail))
			{
				$this->session->set_flashdata('massage','Email Address does not exist');
				redirect('Home/index');
			}
			else
			{
				if($checkemail->password == $password)
				{
					...
					{
						// print_r($checkemail->status); exit;
						$this->session->set_flashdata('warn','Your account is pending for approval');
						redirect('Home/index');
					}
	...
	{
		$this->form_validation->set_rules('email_address','Email ID','required|valid_email');
		$this->form_validation->set_rules('password','Password','required|min_length[8]|max_length[16]' );
		$this->form_validation->set_error_delimiters('<span class="text text-danger">', '</span>');
	}
