...
class Users extends CI_Controller {
	...
	public function index()
	{
		...
		$activeusers = $this->Users_model->GetData("users","","role='User' and status='Active'","created DESC","","5","");
		...
		{
			$getactiveuserchart = $this->Users_model->GetData("users","count(id) as activeuser","month(created)='".$b."' and year(created) = '".date('Y')."' and role='User' and status='Active'","","","","1");
	...
	{
		$getuserdata = $this->Users_model->GetData("users","name,email_address,dob,gender,photo","token ='".base64_decode($id)."'","","","","1");
		if(!empty($getuserdata))
		{
			$data = array(
				...
				'button' => 'Update',
				'action' => site_url('Users/profile_action/'.$id),
				'cancel' => 'Cancel',
