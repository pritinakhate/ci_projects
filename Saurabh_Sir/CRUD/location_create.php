...
if(isset($_POST['save']))
{
	...
	$errors = locationvalidate_form();
	if(empty($errors))
	{
		
		$checklocation = mysqli_fetch_assoc(mysqli_query($conn,"SELECT location FROM locations where location='".$_POST['location']."'"));
		...
		$insertlocations = "INSERT INTO locations set 
		tokenid='".md5("locations_".date('y-m-d-h-i-s').rand(100000,999999))."',
		...
		if(mysqli_query($conn,$insertlocations))
		{
			
		$massage = "Location has been created successfully";
		}
		else
		{
...
<html>
	...
	<body>
		<div class="container">
			...
			<?php if(isset($massage) && ($massage == "Location has been created successfully")) { ?>
